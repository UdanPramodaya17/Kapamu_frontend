...
import { User, Mail, Phone, Shield, Image as ImageIcon } from 'lucide-react';
import DashboardLayout from '../../components/layout/DashboardLayout';
import { authAPI, barberAPI } from '../../api';
...
export default function BarberSettingsPage() {
    ...
    React.useEffect(() => {
        const fetchProfile = async () => {
            try {
                const res = await barberAPI.getMe();
                if (res.data.success) {
                    setMyProfile(res.data.data.barber);
                }
            } catch (err) {
    ...
    const onPortfolioUploadSuccess = async (newImages) => {
        setIsUpdatingPortfolio(true);
        try {
            await barberAPI.updateMe({ portfolioImages: newImages });
            setMyProfile(prev => ({ ...prev, portfolioImages: newImages }));
        } catch (err) {
    ...
    const onAvatarUploadSuccess = async (newImages) => {
        setIsUpdatingPortfolio(true);
        try {
            const avatarUrl = newImages.length > 0 ? newImages[0] : '';
            await barberAPI.updateMe({ avatar: avatarUrl });
            setMyProfile(prev => ({ ...prev, avatar: avatarUrl }));
            if (user) {
                dispatch(setCredentials({ user: { ...user, avatar: avatarUrl } }));
            }
        ...
        <DashboardLayout>
            <div className="max-w-5xl">
                <div className="mb-8">
                    ...
                    <p className="text-gray-500 mt-1">Manage your personal barber profile and security</p>
                </div>
                ...
                {myProfile && (
                    <div className="mt-8 grid lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white border border-black/10 rounded-2xl p-6 shadow-sm">
                            <h2 className="text-xl font-bold text-black mb-6 flex items-center gap-2">
                                <ImageIcon size={20} className="text-black" /> My Profile Photo
                            </h2>
                            <p className="text-sm text-gray-500 mb-6 font-medium">
                                Upload a clear photo of yourself. This is how customers will recognize you.
                            </p>
                            <div className="bg-gray-50 p-4 sm:p-6 rounded-xl border border-black/5">
                                <ImageUploader 
                                    maxImages={1} 
                                    currentImages={myProfile.avatar ? [myProfile.avatar] : []}
                        ...
                        <div className="bg-white border border-black/10 rounded-2xl p-6 shadow-sm">
                            <h2 className="text-xl font-bold text-black mb-6 flex items-center gap-2">
                                <ImageIcon size={20} className="text-black" /> My Portfolio Gallery
                            </h2>
                            <p className="text-sm text-gray-500 mb-6 font-medium">
                                Upload photos of your past work to show customers. These will be visible on the saloon page.
                            </p>
                            
                            <div className="bg-gray-50 p-4 sm:p-6 rounded-xl border border-black/5">
                                <ImageUploader 
                                    maxImages={10} 
                                    currentImages={myProfile.portfolioImages || []}
                                    onUploadSuccess={onPortfolioUploadSuccess}
